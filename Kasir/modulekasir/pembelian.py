...
def pembelian():
    try :
        tanggal = jam()
        data_pengunjung = pd.read_csv("data/daftar_pengunjung.csv")
        daftar_no_order = set(data_pengunjung["No Order"].tolist())
        ...
        for k in range(len(menu_beli2)):
            ...
            list_pembelian = [nomor_order,menu_beli2[k],harga[index],menu_jumlah[menu_beli2[k]],total_harga,tanggal]
...
def save_data_pengunjung(list_pembelian_total):
    read_data = pd.read_csv("data/daftar_pengunjung.csv")
    list_data_pengunjung = read_data.values.tolist()
    
    for data in list_pembelian_total:
        list_data_pengunjung.append(data) 

    saved_data = pd.DataFrame(list_data_pengunjung, columns=["No Order","Nama Menu","Harga","Jumlah","Total Harga","Tanggal"])
    saved_data.to_csv("data/daftar_pengunjung.csv",index=False) 